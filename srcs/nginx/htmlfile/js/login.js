...
async function login(username, password, code_2fa) {
    try {
        const response = await fetch('https://127.0.0.1/api/users/jwtlogin/', {
            method: 'POST',
            headers: {
...
async function loginPage() {
    ...
    intraBtn.addEventListener('click', async function(event) {
        event.preventDefault();
        try {
            window.location.href = "https://api.intra.42.fr/oauth/authorize?client_id=u-s4t2ud-c7fda387fc96d9bc0bbfb60511719a79b6307a19ad79150bd63c9d67d11033fc&redirect_uri=https%3A%2F%2F127.0.0.1%2Fwait&response_type=code";
        } catch (error) {
...
async function intralogin() {
    try {
        ...
        const response = await fetch(`https://127.0.0.1/api/users/login42/${code}/`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
